...
function asU64(input: string): bigint {
  const buf = asArray(input);
  return (BigInt(buf[7]) << 56n) +
    (BigInt(buf[6]) << 48n) +
    (BigInt(buf[5]) << 40n) +
    (BigInt(buf[4]) << 32n) +
    (BigInt(buf[3]) << 24n) +
...
describe('Monocypher WASM Port', function () {
  ...
  //TODO: test noise generation

  it('chacha20hash', function () {
